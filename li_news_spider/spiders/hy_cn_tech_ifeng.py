...
class Ifeng_Spider(CrawlSpider):
    name = 'hy_ifeng'
    ...
    #--------------------------------------------------------
    rules = (Rule(LinkExtractor(allow=r'^https://tech.ifeng.com/./.*'), callback='parse_item', follow=True),
             Rule(LinkExtractor(allow=r'https://tech.ifeng.com/.*'),follow=True),)

    def __init__(self):
        super(Ifeng_Spider, self).__init__(name='hy_ifeng')
        # mysql------------------------------------------
    ...
    def parse_item(self, response):
        ...
        # 正文
        try:
            item['content'] ='\n'.join(response.xpath("//div[@class='text-3w2e3DBc']/p/text() | //div[@id='main_content']/p/text()").extract())
        except Exception:
            item['content'] = ''
        ...
        # 发行时间
        try:
            item['release_time'] = response.xpath("//p[@class='time-1Mgp9W-1']/span/text() | //p[@class='p_time']/span/text()").extract_first().replace('\n','').strip()
        except Exception:
